...
public class DomicilioController {
    ...
    public ResponseEntity<?> getAll() {
        try {
            return ResponseEntity.status(HttpStatus.OK).body("Busqué todos los domicilios");
        } catch (Exception e) {
    ...
    public ResponseEntity<?> getOne(@PathVariable Long id) {
        try {
            return ResponseEntity.status(HttpStatus.OK).body("Busqué este domicilio por Id: " + id);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{\"error\":\"Error, por favor intente más tarde\"}");
        }
    }
    ...
    public ResponseEntity<?> save(@RequestBody Domicilio entity) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body("Grabé el domicilio anterior");
        } catch (Exception e) {
    ...
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody Domicilio entity) {
        ...
        try {
            return ResponseEntity.status(HttpStatus.OK).body("Actualicé los datos del domicilio anterior");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("{\"error\":\"Error, por favor intente más tarde\"}");
        }
    }
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        try {
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Eliminé el domicilio con ID " + id);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("{\"error\":\"Error, por favor intente más tarde\"}");
        }
    }
